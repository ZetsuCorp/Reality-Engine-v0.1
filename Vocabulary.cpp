...
bool json_unescape(
    ...
    while (pos < input.size()) {
        ...
        case 't':
            ...
            break;
        case 'u': {
            // json_escape only ever emits \u00XX for control bytes < 0x20.
            if (pos + 4 > input.size()) {
                return false;
            }
            ...
            const int d0 = hexVal(input[pos]);
            const int d1 = hexVal(input[pos + 1]);
            const int d2 = hexVal(input[pos + 2]);
            const int d3 = hexVal(input[pos + 3]);
            if (d0 < 0 || d1 < 0 || d2 < 0 || d3 < 0) {
                return false;
            }
            const int codepoint = (d0 << 12) | (d1 << 8) | (d2 << 4) | d3;
            if (codepoint > 0xFF) {
                // Not produced by this writer; refuse rather than corrupt.
                return false;
            }
